...
- o remetente SES precisa ser verificado antes de habilitar o modo de entrega `ses`.

## Consequências

- a criação e a evolução da OS não dependem da entrega do e-mail;
- SNS fornece retry assíncrono e DLQ para falhas definitivas;
- o modo `log` preserva o processamento assíncrono sem registrar endereço, assunto ou corpo;
- SES pode exigir verificação do remetente e, no sandbox, também do destinatário;
- a entrega real por e-mail depende das permissões da conta e da confirmação das identidades, ações externas a este código.
